...
const aircraftModel = require('../../model/app/aircraft');
const schema = require('../../schema/app/aircraft')
...
exports.add = async (req, res, next) => {
    ...
    try {
        ...
        if (r) {
            const aircraftInput = new aircraftModel(req.body);
            const response = await aircraftInput.save();
...
exports.list = async (req, res, next) => {
    ...
    try {
        ...
        if (r) {
            const query = {}
            const u = await aircraftModel.find(query).sort({aircraft_no: 1})
            if (u) {
